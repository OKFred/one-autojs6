...
正式环境先一次性安装独立 supervisor，再在 `mobile/` 目录运行守护入口：

```bash
bash node_daemon.sh
```

该脚本现在只启动独立 supervisor。正式版本是不可变 `vX.Y.Z` 制品，环境业务状态相互隔离，启动验证失败自动回滚；迁移后不再执行仓库 `git pull/reset`，旧自更新任务已禁用。
...
- **Query 参数**:
  - `timeout` (number, 可选, 默认 30): 任务执行超时时间（秒）。
- **已弃用**: 该接口返回 HTTP 410。请改用客户端部署接口选择不可变版本和环境，并异步轮询部署状态。
- **返回响应**:
  ```json
  {
    "ok": true,
