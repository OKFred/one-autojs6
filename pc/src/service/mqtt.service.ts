...
export class MqttService {
  ...
  public init(port: number) {
    ...
    if (brokerUrl) {
      console.log(`[MQTT] Connecting to External Cloud EMQX Broker: ${brokerUrl}`);
      this.externalClient = mqtt.connect(brokerUrl, {
        clean: false,
        clientId: 'one_autojs6_pc_server',
        properties: {
      ...
      httpServer.listen(wsPort, () => {
        console.log(`[MQTT-WS] Local Broker WebSocket is running on port ${wsPort}`);
      });
  ...
  private handleTaskResult(payloadStr: string) {
    try {
      ...
      if (taskId && status) {
        console.log(`[MQTT-RESULT] Received task feedback via MQTT for task ${taskId}: ${status}`);
        TaskService.getInstance().updateTaskStatus(taskId, status, message);
      }
    } catch (err: any) {
      console.error('[MQTT-RESULT] Failed to parse task result payload:', err.message);
    }
  }
    ...
    if (this.externalClient) {
      this.externalClient.publish(topic, payloadStr, { qos: 1 }, (err) => {
        if (err) {
          ...
          console.log(`[MQTT] Successfully published to Cloud EMQX topic: ${topic}`);
        }
      });
    } else if (this.aedes) {
      this.aedes.publish({
      ...
      console.error('[MQTT] Neither external client nor local aedes broker is initialized.');
    }
  }
}
