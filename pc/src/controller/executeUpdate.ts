  ...
  try {
    ...
    if (mode !== "download" && mode !== "store" && mode !== "zip") {
      return c.json(
        {
          ok: false,
          message: 'mode must be "download", "store", or "zip"',
          data: {},
        },
        400,
      );
    ...
    if ((mode === "download" || mode === "zip") && !downloadUrl) {
      return c.json(
        {
          ok: false,
          message: 'downloadUrl is required when mode is "download" or "zip"',
          data: {},
        },
        400,
      );
    ...
    const task =
      mode === "download"
        ? await autojsService.dispatchTask(
            "app.install",
            { downloadUrl },
            timeout,
          )
    ...
    console.error("[HTTP] Error creating execute update task:", error);
