...
function findAndClick(keywords, timeoutMs, exact) {
  ...
  while (Date.now() - start < timeoutMs) {
    for (var i = 0; i < keywords.length; i++) {
      ...
      var target =
        textMatches(pattern).findOne(80) || descMatches(pattern).findOne(80);
...
function dismissTransientDialogs() {
  findAndClick(
    ["暂时不要", "以后再说", "Not now", "Maybe later", "Later"],
    500,
    true,
  );
  if (
    textContains("及时接收新互动的通知").exists() ||
    textContains("开启通知，第一时间").exists()
  ) {
...
function enforcePostingLimits(state) {
  ...
  if (request.maxPostsPerDay > 0 && todayCount >= request.maxPostsPerDay) {
    throw new Error(
      "Daily post limit reached: " + todayCount + "/" + request.maxPostsPerDay,
    );
  }
  var elapsedSeconds = Math.floor((now - lastPublishedAt) / 1000);
  if (
    lastPublishedAt > 0 &&
    request.minIntervalSeconds > 0 &&
    elapsedSeconds < request.minIntervalSeconds
  ) {
    throw new Error(
      "Post cooldown active; retry after " +
        (request.minIntervalSeconds - elapsedSeconds) +
        " seconds",
    );
  }
}
...
function isSupportedMedia(filePath) {
  return (
    /\.(jpe?g|png|webp|mp4|mov|m4v)$/i.test(filePath) &&
    filePath.indexOf("tiktok_auto_upload_tmp") < 0
  );
}
...
function collectMaterialCandidates() {
  ...
  if (
    !candidates.length &&
    materialDir &&
    files.exists(materialDir) &&
    files.isDir(materialDir)
  ) {
    var names =
      files.listDir(materialDir, function (name) {
        return isSupportedMedia(name);
      }) || [];
    for (var i = 0; i < names.length; i++)
      candidates.push(files.join(materialDir, names[i]));
  }
...
function openMediaInTikTok(mediaPath) {
  ...
  intent.putExtra(
    ...
    android.net.Uri.fromFile(new java.io.File(mediaPath)),
  );
...
function reachCaptionEditor() {
  ...
  if (!className("android.widget.EditText").exists())
    throw new Error("Caption editor did not appear");
}
...
function enterCaption() {
  ...
  if (!inputs || inputs.length === 0)
    throw new Error("No editable caption field found");
  var success;
  if (inputs.length >= 2 && selectedTitle && selectedDetails) {
    ...
    success =
      success && inputs.length >= 2 && setInputText(inputs[0], selectedTitle);
  } else {
...
function isShareSheetOpen() {
  return (
    textMatches("(?i)^(分享到|Share to)$").exists() ||
    textContains("复制链接").exists() ||
    textContains("Copy link").exists()
  );
}
...
function waitForPublishCompletion() {
  ...
  while (Date.now() - start < 90000) {
    ...
    if (
      textMatches(
        "(?i).*(上传失败|发布失败|Couldn't upload|Post failed|重试).*",
      ).exists()
    ) {
...
function navigateToProfile() {
  ...
  if (
    !isOnOwnProfileGrid() &&
    descMatches("(?i).*(分享视频|Share).*").exists() &&
    (desc("返回").exists() || desc("Back").exists())
  ) {
    back();
    pause(1800, 2800);
  }
  if (!isOnOwnProfileGrid()) {
    ...
    if (!profileTab || !performClick(profileTab))
      throw new Error("Profile navigation button not found");
    pause(3000, 5000);
  }
  dismissTransientDialogs();
  swipe(
    device.width / 2,
    Math.floor(device.height * 0.35),
    device.width / 2,
  ...
  if (handleNode)
    profileHandle = String(handleNode.text() || "")
      .replace(/^@/, "")
      .trim();
  if (!profileHandle)
    throw new Error("Current TikTok profile handle could not be read");
}
...
function isOnOwnProfileGrid() {
  return (
    id(packageName + ":id/se1").exists() &&
    id(packageName + ":id/tv_play_count").exists()
  );
}
...
function clickLatestProfileTile() {
  ...
  for (var i = 1; i < playCounts.length; i++) {
    if (playCounts[i].bounds().left < first.bounds().left)
      first = playCounts[i];
  }
  var current = first;
  while (current && !current.isClickable()) current = current.parent();
  ...
  return click(
    bounds.centerX(),
    Math.max(Math.floor(device.height * 0.42), bounds.top - 160),
  );
}
...
function openLatestPost() {
  for (var attempt = 0; attempt < 3; attempt++) {
    if (!isOnOwnProfileGrid())
      throw new Error(
        "Left the current account profile before opening the latest post",
      );
    if (!clickLatestProfileTile())
      throw new Error("Latest profile tile could not be clicked");
    pause(2200, 3400);
...
function findShareButton(timeoutMs) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    for (var i = 0; nodes && i < nodes.length; i++) {
      var bounds = nodes[i].bounds();
      if (
        bounds.centerX() > device.width * 0.72 &&
        bounds.centerY() > device.height * 0.35
      )
        return nodes[i];
    }
    sleep(300);
...
function copyPostUrl() {
  ...
  for (var attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (!shareButton || !performClick(shareButton))
      throw new Error("Share button not found on latest post");
    pause(1500, 2500);
    var shareSheetOpen = isShareSheetOpen();
    var clickedCopy = findAndClick(["复制链接", "Copy link"], 2500, false);
    if (!clickedCopy && shareSheetOpen) {
      clickedCopy = click(
        Math.floor(device.width * 0.105),
        Math.floor(device.height * 0.765),
      );
    ...
    if (clickedCopy) {
      ...
      if (url) return url;
      reportProgress(
        "TikTok is still processing the post; retrying link copy (" +
          (attempt + 1) +
          "/" +
          maxAttempts +
          ").",
      ...
      reportProgress(
        "Copy link action was unavailable; retrying (" +
          (attempt + 1) +
          "/" +
          maxAttempts +
          ").",
...
function retrieveLatestPostUrl() {
  ...
  if (!postUrl) {
    ...
    return {
      ...
      error:
        "Latest post is still processing or no valid TikTok URL was copied",
    };
  }
...
function runTikTokPost() {
  ...
  if (!materials.length)
    throw new Error("No supported media found in the supplied material pool");
  selectedMediaPath =
    request.videoPath || request.imagePath
      ? materials[0]
      : chooseLeastRecentlyUsed(materials, state.materialUses);
  selectedMediaType = /\.(mp4|mov|m4v)$/i.test(selectedMediaPath)
    ? "video"
    : "image";
  ...
  if (!findAndClick(["发布", "Post"], 10000, true))
...
try {
  ...
  taskResult = JSON.stringify(
    request.linkOnly ? retrieveLatestPostUrl() : runTikTokPost(),
  );
} catch (error) {
  taskStatus = "FAILURE";
  taskResult = JSON.stringify({
