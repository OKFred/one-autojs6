...
if (targetPath.indexOf("/sdcard/") !== 0) {
  throw new Error("targetPath must stay under /sdcard/");
}
...
try {
  ...
  while ((len = input.read(buffer)) !== -1) {
    ...
    if (
      totalBytes > 0 &&
      downloadedBytes - lastReported >= totalBytes * 0.01 &&
      ...
      lastReportTime = now;
      reportProgress(
        "下载中... 已下载 " + (downloadedBytes / 1024 / 1024).toFixed(2) + "MB",
      );
    }
  }
