...
var downloadUrl = String(request.downloadUrl || "");
...
while ((len = input.read(buffer)) !== -1) {
  ...
  if (
    totalBytes > 0 &&
    downloadedBytes - lastReported >= totalBytes * 0.05 &&
    now - lastReportTime > 2000
  ) {
...
reportProgress("开始安全解压ZIP文件...");
{
  var zipFile = new java.util.zip.ZipFile(zipPath);
  var entries = zipFile.entries();
  var canonicalDest =
    new java.io.File(destDir).getCanonicalPath() + java.io.File.separator;
  while (entries.hasMoreElements()) {
    var entry = entries.nextElement();
    if (!entry.isDirectory()) {
      var is = zipFile.getInputStream(entry);
      var outFile = new java.io.File(destDir, entry.getName());
      var canonicalOutput = outFile.getCanonicalPath();
      if (canonicalOutput.indexOf(canonicalDest) !== 0) {
        is.close();
        zipFile.close();
        throw new Error("Unsafe ZIP entry: " + entry.getName());
      }
...
shell(
  "cp '" + apkPath + "' '" + tmpPath + "' && chmod 777 '" + tmpPath + "'",
  true,
);
