...
var storePackage = String(request.storePackage || "");

if (
  !/^[A-Za-z0-9._]+$/.test(packageName) ||
  (storePackage && !/^[A-Za-z0-9._]+$/.test(storePackage))
) {
...
for (var i = 0; i < 20; i++) {
  for (var j = 0; j < updateKeywords.length; j++) {
    var btn =
      textContains(updateKeywords[j]).findOne(50) ||
      descContains(updateKeywords[j]).findOne(50);
    if (btn) {
      clickedBtn = btn;
      foundState = "update";
      break;
  ...
  for (var j = 0; j < openKeywords.length; j++) {
    var btn =
      textContains(openKeywords[j]).findOne(50) ||
      descContains(openKeywords[j]).findOne(50);
    if (btn) {
      clickedBtn = btn;
      foundState = "open";
      break;
  ...
  for (var j = 0; j < installKeywords.length; j++) {
    var btn =
      textContains(installKeywords[j]).findOne(50) ||
      descContains(installKeywords[j]).findOne(50);
    if (btn) {
      clickedBtn = btn;
      foundState = "install";
      break;
...
if (clickedBtn) {
  ...
  throw new Error(
    "Timeout (20s): 无法在应用商店页面中找到相关的[更新/打开/安装]按钮。如果是特殊应用商店，请考虑使用静默下载安装(mode=download)。",
  );
}
